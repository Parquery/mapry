...
void some_graph_from(
  ...
  if (!value.isMember("array_of_booleans")) {
    ...
    if (!value_0.isArray()) {
      ...
      errors->add(
        std::string(ref)
          .append("/array_of_booleans"),
        message(
          expected_but_got,
          strlen(expected_but_got),
          value_type_to_string(
            value_0.type())));
    } else if (value_0.size() > 1) {
      constexpr auto expected_but_got(
      ...
      errors->add(
        ...
        message(
          expected_but_got,
          strlen(expected_but_got),
          std::to_string(value_0.size())));
    } else {
      std::vector<bool>& target_0 = target->array_of_booleans;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_booleans;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = vector_0[i_0];
  }
  some_graph_as_value["array_of_booleans"] = std::move(target_0);
