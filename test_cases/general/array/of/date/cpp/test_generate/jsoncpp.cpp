...
void some_graph_from(
  ...
  if (!value.isMember("array_of_dates")) {
    ...
    if (!value_0.isArray()) {
      ...
      std::vector<struct tm>& target_0 = target->array_of_dates;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_dates;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
  ...
  some_graph_as_value["array_of_dates"] = std::move(target_0);
