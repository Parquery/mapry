...
void some_graph_from(
  ...
  if (!value.isMember("array_of_datetimes")) {
    ...
    if (!value_0.isArray()) {
      ...
      std::vector<struct tm>& target_0 = target->array_of_datetimes;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_datetimes;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = tm_to_string(
      vector_0[i_0],
      "%Y-%m-%dT%H:%M:%SZ");
  }
  some_graph_as_value["array_of_datetimes"] = std::move(target_0);
