...
void some_graph_from(
  ...
  if (not value.isMember("array_of_durations")) {
    ...
    if (not value_0.isArray()) {
      ...
      std::vector<std::chrono::nanoseconds>& target_0 = target->array_of_durations;
      ...
      for (const Json::Value& item_0 : value_0) {
        if (not item_0.isString()) {
          ...
          const std::string cast_1_str = item_0.asString();
          std::string error_1;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_durations;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = duration_to_string(vector_0[i_0]);
  }
  some_graph_as_value["array_of_durations"] = std::move(target_0);
