...
void some_graph_from(
  ...
  if (not value.isMember("array_of_floats")) {
    ...
    if (not value_0.isArray()) {
      ...
      std::vector<double>& target_0 = target->array_of_floats;
      ...
      for (const Json::Value& item_0 : value_0) {
        if (not item_0.isDouble()) {
          ...
          target_0.at(i_0) = item_0.asDouble();
        }
        ++i_0;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_floats;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = vector_0[i_0];
  }
  some_graph_as_value["array_of_floats"] = std::move(target_0);
