...
void some_graph_from(
  ...
  if (!value.isMember("array_of_integers")) {
    ...
    if (!value_0.isArray()) {
      ...
      std::vector<int64_t>& target_0 = target->array_of_integers;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_integers;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = vector_0[i_0];
  }
  some_graph_as_value["array_of_integers"] = std::move(target_0);
