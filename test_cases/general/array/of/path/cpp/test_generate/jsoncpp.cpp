...
void some_graph_from(
  ...
  if (not value.isMember("array_of_paths")) {
    ...
    if (not value_0.isArray()) {
      ...
      std::vector<boost::filesystem::path>& target_0 = target->array_of_paths;
      ...
      for (const Json::Value& item_0 : value_0) {
        if (not item_0.isString()) {
          ...
          errors->add(
            ...
            target_0.at(i_0) = boost::filesystem::path(
            item_0.asString());
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_paths;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = vector_0[i_0].string();
  }
  some_graph_as_value["array_of_paths"] = std::move(target_0);
