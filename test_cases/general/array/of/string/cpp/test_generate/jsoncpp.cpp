...
void some_graph_from(
  ...
  if (not value.isMember("array_of_strings")) {
    ...
    if (not value_0.isArray()) {
      ...
      std::vector<std::string>& target_0 = target->array_of_strings;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_strings;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
    target_0[i_0] = vector_0[i_0];
  }
  some_graph_as_value["array_of_strings"] = std::move(target_0);
