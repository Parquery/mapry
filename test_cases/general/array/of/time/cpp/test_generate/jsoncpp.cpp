...
void some_graph_from(
  ...
  if (not value.isMember("array_of_times")) {
    ...
    if (not value_0.isArray()) {
      ...
      std::vector<struct tm>& target_0 = target->array_of_times;
...
Json::Value serialize_some_graph(
  ...
  const auto& vector_0 = some_graph.array_of_times;
  for (int i_0 = 0;
      i_0 < vector_0.size();
      ++i_0) {
  ...
  some_graph_as_value["array_of_times"] = std::move(target_0);
