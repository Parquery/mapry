...
func SomeGraphToJSONable(
	...
	////
	// Serialize ArrayOfTimeZones
	////

	count0 := len(instance.ArrayOfTimeZones)
	slice0 := instance.ArrayOfTimeZones
	target0 := make([]interface{}, count0)
	for i0 := 0; i0 < count0; i0++ {
		target0[i0] = slice0[i0].String()
	}
	target["array_of_time_zones"] = target0
