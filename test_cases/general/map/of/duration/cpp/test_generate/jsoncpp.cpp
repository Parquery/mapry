    ...
    if (!value_0.isObject()) {
      ...
      for (Json::ValueConstIterator it_0 = value_0.begin(); it_0 != value_0.end(); ++it_0) {
        ...
        if (!value_1.isString()) {
          ...
          const std::string cast_1_str = value_1.asString();
          std::string error_1;
...
Json::Value serialize_some_graph(
  ...
  const auto& map_0 = some_graph.map_of_durations;
  for (const auto& kv_0 : map_0) {
    target_0[kv_0.first] = duration_to_string(kv_0.second);
  }
  some_graph_as_value["map_of_durations"] = std::move(target_0);
