      ...
      for (Json::ValueConstIterator it_0 = value_0.begin(); it_0 != value_0.end(); ++it_0) {
        ...
        if (not value_1.isString()) {
          ...
          errors->add(
            ...
            target_0[it_0.name()] = boost::filesystem::path(
            value_1.asString());
...
Json::Value serialize_some_graph(
  ...
  const auto& map_0 = some_graph.map_of_paths;
  for (const auto& kv_0 : map_0) {
    target_0[kv_0.first] = kv_0.second.string();
  }
  some_graph_as_value["map_of_paths"] = std::move(target_0);
